...
@celery.task()
def run_model_from_id(modelrun_id):
    print 'hello'
    ...
    # create a Session
    dbsession = Session()
    modelrun = dbsession.query(ModelRun).get(modelrun_id)
    run_model(dbsession,modelschemas,model_modules,modelrun)
...
def create_resource_mapping(dbsession,schema,modelrun):
    resources = dbsession.query(ModelResource).filter_by(modelrun_id=modelrun.id).all()
    mapping={}
...
def add_progress_event(dbsession,event):
  existing_event = dbsession.query(
...
def run_model(dbsession,modelschemas,model_modules,modelrun):
    mapping = create_resource_mapping(dbsession,modelschemas[modelrun.model_name],modelrun)
